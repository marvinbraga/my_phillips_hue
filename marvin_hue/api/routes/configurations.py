...
async def apply_configuration(
    ...
    if manager is None:
        raise HTTPException(status_code=503, detail="Gerenciador de configurações não disponível")

    # Valida nome da configuração
    if not request.config_name:
        raise HTTPException(status_code=400, detail="Nome da configuração é obrigatório")
    ...
    try:
        ...
        logger.error(f"Unexpected error applying config '{request.config_name}': {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Erro ao aplicar configuração: {str(e)}")
